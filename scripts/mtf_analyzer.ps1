# mtf_analyzer.ps1 - Multi-TimeFrame Confluence analyzer for crypto.
# Validates Daily trend, 4H momentum, and 1H pullback entry alignment.
# Usage: .\mtf_analyzer.ps1 -Symbol "BTC/USD"
...
function Get-Bars($sym, $tf, $limit) {
    # Days of lookback per timeframe so we always get enough bars
...
function Get-SwingLow($bars, $lookback = 30) {
    ...
    for ($i = $start + 1; $i -le $bars.Count - 2; $i++) {
        if ([double]$bars[$i].l -lt [double]$bars[$i-1].l -and [double]$bars[$i].l -lt [double]$bars[$i+1].l) {
            if ([double]$bars[$i].l -gt $lowest -or $lowest -eq [double]$bars[$start].l) {
                $lowest = [double]$bars[$i].l
            }
        }
    }
    return $lowest
...
# ---- BTC MACRO FILTER (always check BTC regardless of pair) -----------------
function Get-BtcFilter {
    try {
        $btcBars = Get-Bars "BTC/USD" "1Day" 60
        $btcClose = $btcBars | ForEach-Object { [double]$_.c }
        $btcEma50 = (Calc-EMA $btcClose 50)[-1]
        $btcPrice = $btcClose[-1]
        return @{ bullish=($btcPrice -gt $btcEma50); btc_price=$btcPrice; btc_ema50=[math]::Round($btcEma50,2) }
    } catch { return @{ bullish=$true; btc_price=0; btc_ema50=0 } }
}
...
try {
    ...
    $h1    = Get-Bars $Symbol "1Hour" 120

    if ($daily.Count -lt 30 -or $h4.Count -lt 30 -or $h1.Count -lt 30) {
    ...
    # ---- BTC MACRO FILTER ----
    $btc = if ($Symbol -ne "BTC/USD") { Get-BtcFilter } else { @{ bullish=$true; btc_price=$curPrice; btc_ema50=0 } }
    ...
    $dRsiOk    = $dRsi -ge 40 -and $dRsi -le 70   # in trend, not overbought
    ...
    # Recent 4H higher-low pattern (bullish structure)
    $h4Lows = @()
    ...
    $h1Pullback  = $h1Rsi -ge 35 -and $h1Rsi -le 60   # pulled back, not oversold
    $h1NotTop    = $h1Rsi -le 68                        # hard disqualifier

    # Volume surge on recent 1H bar (selling exhaustion / buyers stepping in)
    ...
    # BTC macro (prerequisite ??? counted in score so borderline setups lose points)
    if (-not $btc.bullish)  { $score -= 3.0; $reasons += "BTC below EMA50 (bearish macro)" }
    ...
    if ($h4HigherLow)       { $score += 0.5; $reasons += "4H higher-low structure" }

    # 1H layer (max 3)
    if ($h1NearEma)         { $score += 1.5; $reasons += "1H price at EMA20 ($([math]::Round($h1Ema20,4)))" }
    if ($h1Pullback)        { $score += 1.0; $reasons += "1H RSI $([math]::Round($h1Rsi,1)) (pullback zone)" }
    if ($h1VolR -ge 1.2)    { $score += 0.5; $reasons += "1H volume $([math]::Round($h1VolR,1))x (interest)" }

    # ---- STOP using 1H swing low (tighter than daily) ----
    $h1SwingLow  = Get-SwingLow $h1 25
    $stopFromSwing = if ($h1SwingLow -gt 0 -and $h1SwingLow -lt $curPrice) { $h1SwingLow * 0.997 } else { 0 }
    $stopFromAtr   = $curPrice - $h1Atr * 1.5

    # Use swing low stop if it's within 4% of price (tight enough to be useful)
    ...
    $t1Price     = [math]::Round($curPrice + $stopDist * 1.0, $dp)
    $t2Price     = [math]::Round($curPrice + $stopDist * 2.0, $dp)
    $stopPrice   = [math]::Round($stopPrice, $dp)
    $stopLimPrice= [math]::Round($stopPrice * 0.9975, $dp)
    $rrRatio     = if ($stopDist -gt 0) { [math]::Round($stopDist * 2.0 / $stopDist, 1) } else { 0 }
    ...
    @{
        ...
        signal        = $signal
        price         = $curPrice
        btc_bullish   = $btc.bullish
        daily_ema20   = [math]::Round($dEma20, $dp)
        daily_ema50   = [math]::Round($dEma50, $dp)
        daily_rsi     = [math]::Round($dRsi, 1)
        h4_ema20      = [math]::Round($h4Ema20, $dp)
