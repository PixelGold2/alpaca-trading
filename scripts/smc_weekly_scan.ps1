# smc_weekly_scan.ps1 - Deep SMC bullish reversal scanner on WEEKLY timeframe
...
function Get-WeeklyBars($sym) {
    # ~130 weeks = 2.5 years of weekly data
    $start = (Get-Date).AddDays(-910).ToUniversalTime().ToString("yyyy-MM-ddT00:00:00Z")
    $url = "https://data.alpaca.markets/v2/stocks/$sym/bars?timeframe=1Week&start=$start&limit=130&feed=iex&adjustment=raw"
    try {
        $r = Invoke-RestMethod -Uri $url -Method Get -Headers $hdr -TimeoutSec 15
        return $r.bars
    } catch { return $null }
}
...
function Find-BullishOB($bars, $swingHighs) {
    ...
    foreach ($sh in ($swingHighs | Sort-Object idx -Descending)) {
        # Weekly OB: last bearish candle within 8 weeks before the swing high
        for ($i = $sh.idx - 1; $i -ge [math]::Max(0, $sh.idx - 8); $i--) {
...
function Find-LiquiditySweep($bars, $swingLows, $lookback) {
    $n = $bars.Count
    $candidates = $swingLows | Where-Object { $_.idx -ge ($n - $lookback - 4) -and $_.idx -le ($n - 2) }
...
function Find-CHoCH($bars, $swingHighs, $closes, $ema21s) {
    ...
    # On weekly: require at least 8 of last 20 weeks below EMA21 (more lenient for weekly)
    if ($belowCt -lt 8) { return @{ found=$false } }
    # Look for recent swing high that price just broke above (within last 20 weekly bars)
    $recentSHs = $swingHighs | Where-Object { $_.idx -ge ($n-20) -and $_.idx -le ($n-2) }
...
function Find-DemandZone($bars, $atr) {
    ...
    # Weekly demand zones: big bullish weekly candle, price retested the base
    for ($i = [math]::Max(2, $n-25); $i -lt $n-2; $i++) {
        $body = [math]::Abs([double]$bars[$i].c - [double]$bars[$i].o)
        if ([double]$bars[$i].c -gt [double]$bars[$i].o -and $body -gt $atr * 1.0) {
            $dzHigh = [double]$bars[$i].o + $atr * 0.2
...
function Get-PD($bars, $lookback) {
    # Weekly PD: use 52-week range (1 year)
...
$results = @()
Write-Host ""
Write-Host "  SMC Reversal Scan | $(Get-Date -Format 'yyyy-MM-dd HH:mm') | WEEKLY bars (~130 weeks)" -ForegroundColor Cyan
...
foreach ($sym in $symbols) {
    ...
    # Volume: weekly avg of prior 10 weeks
    ...
    # Weekly swing lookback = 2 (requires 2 bars each side)
    $swingLos = Get-SwingLows  $bars 2
    $swingHis = Get-SwingHighs $bars 2

    # Weekly-tuned lookbacks
    $ob    = Find-BullishOB       $bars $swingHis
    $fvgs  = Find-BullishFVGs     $bars 20   # 20 weekly bars ~ 5 months
    $sweep = Find-LiquiditySweep  $bars $swingLos 10  # 10-week sweep window
    $choch = Find-CHoCH           $bars $swingHis $closes $ema21s
    $dz    = Find-DemandZone      $bars $atr
    $pdPct = Get-PD               $bars 52   # 52-week (1-year) range
    ...
    if    ($pdPct -lt 35) { $score += 1.0; $sigs += "52wk-DeepDisc($($pdPct)%)" }
    elseif ($pdPct -lt 50) { $score += 0.5; $sigs += "52wk-Disc($($pdPct)%)" }

    if ($volRatio -gt 1.5 -and ($inOB -or $inFVG -or $sweep.found)) {
        $score += 1.0; $sigs += "Vol-$([math]::Round($volRatio,1))x"
    }

    if    ($curP -gt $ema21)                    { $emaCtx = "P>EMA21w" }
    elseif ($curP -gt $ema50 -and $ema50 -gt 0) { $emaCtx = "P>EMA50w" }
...
Write-Host "  SMC WEEKLY REVERSAL SCAN  |  $(Get-Date -Format 'yyyy-MM-dd HH:mm')" -ForegroundColor Yellow
Write-Host "  52-week premium/discount | 130 weeks of data | lb=2 weekly swings" -ForegroundColor DarkGray
Write-Host "======================================================================" -ForegroundColor Cyan
...
if ($prime.Count -gt 0) {
    Write-Host ""
    Write-Host "  [PRIME] Score >= 10 -- maximum SMC confluence on weekly" -ForegroundColor Magenta
    $prime | Format-Table Sym,Score,Price,RSI,"52wk%",EMA,Signals -AutoSize -Wrap
}
if ($strong.Count -gt 0) {
    Write-Host "  [STRONG] Score 7.5-9.9" -ForegroundColor Green
    $strong | Format-Table Sym,Score,Price,RSI,"52wk%",EMA,Signals -AutoSize -Wrap
}
if ($good.Count -gt 0) {
    Write-Host "  [GOOD] Score 5.0-7.4" -ForegroundColor Yellow
    $good | Format-Table Sym,Score,Price,RSI,"52wk%",EMA,Signals -AutoSize -Wrap
}
if ($watch.Count -gt 0) {
    Write-Host "  [WATCH] Score 3.0-4.9" -ForegroundColor DarkYellow
    $watch | Format-Table Sym,Score,Price,RSI,"52wk%",EMA,Signals -AutoSize -Wrap
}
